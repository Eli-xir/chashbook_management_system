...
export function creditDocument(overview: UserOverview, userName: string): ReportDocument {
  ...
  return { title: userName, subtitle: 'Credits received · All amounts in PKR', columns: ['Date/time', 'Head', 'Category', 'Credit (PKR)', 'Received to date (PKR)'], pages };
}
...
const numeric = (value: string | number) => typeof value === 'number' ? value.toLocaleString('en-PK', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : value;
...
export async function ledgerFile(report: ReportDocument, format: ReportFormat): Promise<File> {
  ...
  if (format === 'excel') {
    ...
    sheet.mergeCells(4, 2, 4, last); sheet.getCell(4, 2).value = 'All amounts in PKR';
    ...
    report.pages.forEach((page, index) => {
      ...
      page.forEach((cells) => {
        ...
        row.eachCell((cell) => { if (typeof cell.value === 'number') { cell.numFmt = '#,##0.00;[Red]-#,##0.00'; cell.alignment = { horizontal: 'right', vertical: 'top' }; } });
      });
    });
  ...
  }
  {
    ...
    report.pages.forEach((page, index) => {
      ...
      autoTable(pdf, { startY: tableTop, head: [report.columns], body,
        ...
        willDrawPage: () => {
          ...
          pdf.setFontSize(7); pdf.setTextColor(100); pdf.text('PKR', 198, 15, { align: 'right' });
    ...
    for (let page = 1; page <= pdf.getNumberOfPages(); page++) {
      ...
      pdf.text('Sohail Malik Architects · All amounts in PKR', 12, 286);
